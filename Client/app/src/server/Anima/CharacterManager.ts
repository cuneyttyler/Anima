...
export default class CharacterManager {
    ...
    GetPersonalityText(character) {
        ...
        if(!character.personality.kind) {
            ...
            text += "Exhibits extremely rude behavior, with a high level of hostility and a complete lack of empathy. Often acts in ways that are intentionally harmful and disregards the well-being of others."
        } else if(character.personality.kind >= -75 && character.personality.kind < -50 ) {
            text += "Shows severe rudeness, with frequent acts of unkindness and a general disregard for the feelings of others. Actions are often hurtful, though they may not always be deliberately malicious."
        } else if(character.personality.kind >= -50 && character.personality.kind < -25 ) {
            text += "Demonstrates moderate rudeness. Can be inconsiderate and sometimes engages in behavior that may be hurtful, but actions are not consistently harmful. May struggle with empathy at times."
        } else if(character.personality.kind >= -25 && character.personality.kind < 0 ) {
            text += "Exhibits slight rudeness. Generally means well, but behavior can occasionally come across as insensitive or thoughtless. May have moments where actions negatively impact others, though not intentionally."
        } else if(character.personality.kind >= 0 && character.personality.kind < 25 ) {
            text += "Shows slight kindness. Often acts in considerate and thoughtful ways, though may occasionally fall short in fully understanding others' needs. Behavior is generally pleasant but not always impactful."
        } else if(character.personality.kind >= 25 && character.personality.kind < 50 ) {
            text += "Displays moderate kindness. Frequently engages in kind and supportive actions, showing empathy and concern for others. Behavior is generally uplifting and contributes positively to the environment."
        } else if(character.personality.kind >= 50 && character.personality.kind < 75 ) {
            text += "Demonstrates high kindness. Consistently kind, generous, and supportive, often going out of the way to help others and create a positive atmosphere. Actions are marked by genuine empathy and care."
        } else if(character.personality.kind >= 75) {
            text += "Embodies extreme kindness. Behavior is characterized by exceptional kindness, selflessness, and compassion. Always strives to uplift others and is deeply committed to the well-being of those around, often going beyond what is expected."
        } else if(character.personality.kind == 100) {
            text += "Embodies unparalleled kindness and compassion. Exhibits extraordinary selflessness, always going out of the way to uplift and support others. Acts with deep empathy and dedication, consistently making a profound positive impact on everyone around."
        }
        
        if(!character.personality.positive) {
            text += ""
        ...
        if(!character.personality.peaceful) {
            text += ""
        ...
        if(!character.personality.extravert) {
            text += ""
    ...
    GetAliveCharacterList() {
        ...
        for(let i in ALIVE_CHARACTERS) {
            let character = CHARACTERS.find((c) => c.name == ALIVE_CHARACTERS[i].name)
    ...
    GetCharacter(name) {
        ...
        for(let i in CHARACTERS) {
            if(!CHARACTERS[i].id) {
                console.error("Character doesn't have id: " + JSON.stringify(CHARACTERS[i]))
                continue
            }
